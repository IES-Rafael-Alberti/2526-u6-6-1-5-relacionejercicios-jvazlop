...
1) ¿Qué comportamiento concreto violaba LSP?
 RESPUESTA:  
El comportamiento concreto que violaba LSP en la versión v0 era el método guardar() en RepositorioSoloLecturaV0.

RepositorioSoloLecturaV0 hereda de RepositorioPersonasV0, por lo que un cliente que espera poder guardar y luego buscar personas puede usarse como si fuera un repositorio normal

UnsupportedOperationException rompe la sustitución:un objeto de la subclase no se puede usar como sustituto del tipo base sin cambiar el comportamiento
...
BuscadorPersonas – solo lectura, define buscar(nombre: String)

GuardadorPersonas – solo escritura, define guardar(persona: Persona)

RepositorioPersonas implementa ambas interfaces, por lo que puede leerse y escribirse normalmente

RepositorioSoloLectura implementa solo BuscadorPersonas, eliminando la obligación de implementar guardar()

3) ¿Cómo lo demostrarías con una prueba/ejemplo?

RESPUESTA:

https://github.com/IES-Rafael-Alberti/2526-u6-6-1-5-relacionejercicios-jvazlop/blob/6c646c0833219a5b6be7ec50a14ca75e7eed1643/src/main/kotlin/es/ies/ejercicios/u6/ej65/lsp/LspV0.kt#L61-L71

RepositorioPersonas puede sustituir al tipo esperado por un cliente que necesita lectura y escritura, cumpliendo el contrato

RepositorioSoloLectura puede sustituir al tipo esperado por un cliente que solo necesita lectura, sin lanzar errores

Ningún cliente intenta usar un método que la clase no implementa
...
guardar(persona: Persona) → el cliente que solo necesitaba buscar personas (BuscadorPersonasV0) no lo usaba

exportarCsv() → el cliente que solo buscaba datos no necesitaba exportar nada

borrarTodo() → tampoco era necesario para clientes que solo querían leer información
...
https://github.com/IES-Rafael-Alberti/2526-u6-6-1-5-relacionejercicios-jvazlop/blob/a5690652f2ebe46f508b30f91862e04204cefa8b/src/main/kotlin/es/ies/ejercicios/u6/ej65/isp/IspV0.kt#L4-L28
...
Código más claro
Cada interfaz tiene una responsabilidad única, por lo que los clientes dependen solo de lo que necesitan, haciendo el código más fácil de entender

Cumplimiento de LSP
No es necesario implementar métodos que no se usan ni lanzar UnsupportedOperationException, lo que evita errores al sustituir una clase por otra

Mayor flexibilidad y reutilizacion
Se pueden crear nuevos tipos de repositorios (por ejemplo, solo lectura, solo escritura, remoto) sin afectar a los clientes existentes

Facilita el mantenimiento y extension
Cambiar una capacidad (por ejemplo, la forma de exportar CSV) no afecta a los clientes que solo leen o escriben datos, reduciendo riesgos de introducir errores

Testeo mas sencillo
Cada cliente se puede probar con la interfaz mínima que necesita, haciendo tests unitarios más claros y específicos
...
https://github.com/IES-Rafael-Alberti/2526-u6-6-1-5-relacionejercicios-jvazlop/blob/ab6bc7f5d31b5af571946823456ace2cade46afe/src/main/kotlin/es/ies/ejercicios/u6/ej65/dip/DipV0.kt#L11-L13

Define solo el comportamiento necesario que el módulo de alto nivel (ControladorInformes) requiere: generar un informe a partir de un título y una lista de elementos Resumible.

No especifica cómo se genera el informe, dejando que las implementaciones concretas decidan los detalles.

3) ¿Cómo intercambiarías implementaciones sin tocar el módulo de alto nivel?

RESPUESTA:

Puedo intercambiar implementaciones simplemente inyectando otra clase que implemente GeneradorInforme al construir el módulo de alto nivel. No es necesario modificar la clase ControladorInformes.
